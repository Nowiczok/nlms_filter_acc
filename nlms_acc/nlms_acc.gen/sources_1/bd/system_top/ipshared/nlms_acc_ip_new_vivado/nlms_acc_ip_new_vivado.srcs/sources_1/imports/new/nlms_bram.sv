...
//  `endif  // BRAM_SYNTH
  ...
  // write data process
//  `ifdef BRAM_SYNTH
     logic [WORD_WIDTH-1:0] mem_content [HEIGHT];  // this version is recomended by Xilinx docs
...
//  `endif  // BRAM_SYNTH
  ...
  always @(posedge clk) begin
    if (re) begin
      for(integer i = 0; i < RD_PORT_NUM_WORDS; i += 1) begin
        rdata[i] <= mem_content[raddr_aligned + i];
      end
    end
  end 
