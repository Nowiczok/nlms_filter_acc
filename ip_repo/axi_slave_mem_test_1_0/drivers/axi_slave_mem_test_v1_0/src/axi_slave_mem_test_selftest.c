...
XStatus AXI_SLAVE_MEM_TEST_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  AXI_SLAVE_MEM_TEST_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = AXI_SLAVE_MEM_TEST_mReadMemory(baseaddr+4*Index);
