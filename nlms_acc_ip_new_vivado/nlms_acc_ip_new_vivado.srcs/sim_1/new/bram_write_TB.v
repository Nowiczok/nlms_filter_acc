...
initial begin
  ...
  #CLK_PERIOD;
  // load data to x buff
  for(integer i = X_BUFF_BASE_ADDR; i < X_BUFF_BASE_ADDR+NUM_X_SAMPLES; i += 1) begin
    bram_en_a = 1'b1;
    bram_we_a = 4'hf;
  ...
  // load data to d buff
  for(integer i = D_BUFF_BASE_ADDR; i < D_BUFF_BASE_ADDR+NUM_X_SAMPLES; i += 1) begin
    bram_en_a = 1'b1;
    bram_we_a = 4'hf;
    bram_addr_a = i[BRAM_ADDR_WIDTH-1:0];
  ...
  // load x fifo buff
  for(integer i = X_FIFO_BUFF_BASE_ADDR; i < X_FIFO_BUFF_BASE_ADDR+NUM_H_COEFS; i += 1) begin
    bram_en_a = 1'b1;
    bram_we_a = 4'hf;
    bram_addr_a = i[BRAM_ADDR_WIDTH-1:0];
    bram_wrdata_a = '0;
  ...
  // load h buff
  for(integer i = H_BUFF_BASE_ADDR; i < H_BUFF_BASE_ADDR+NUM_H_COEFS; i += 1) begin
    bram_en_a = 1'b1;
    bram_we_a = 4'hf;
    bram_addr_a = i[BRAM_ADDR_WIDTH-1:0];
